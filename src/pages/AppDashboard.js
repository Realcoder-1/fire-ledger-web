...
// ── Smart CSV parser ─────────────────────────────────────────────────
// Detects column meaning by fuzzy-matching headers regardless of name/order
function smartParseCSV(text) {
  ...
  // Find which line is the header (has alphabetic content)
  let headerIdx = 0;
  ...
  // If no explicit type column, we'll infer from amount sign or debit/credit columns
  ...
  const rows = dataLines.map(line => {
    ...
    // Handle debit/credit split columns (bank statement style)
    ...
    // If amount is negative and no type, it's a need; positive = income
    ...
    // Date normalisation — try multiple formats
