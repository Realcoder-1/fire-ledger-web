...
const makeFmt = (currency) => {
  ...
  return {
    fmt: n => `${symbol}${Math.abs(parseFloat(n)).toLocaleString(undefined, { maximumFractionDigits: 0 })}`,
...
function GuidePage() {
  ...
  return (
    <div className="fl-page">
      <div className="fl-page-top"><div><h1 className="fl-title">Guide</h1><p className="fl-subtitle">Everything you need to know to get the most out of FIRE Ledger</p></div></div>
        ...
        <nav className="fl-guide-nav">
          {sections.map((s, i) => (
            <button key={i} className={`fl-guide-nav-item ${active === i ? 'active' : ''}`} onClick={() => setActive(i)}>
              <span className="fl-guide-nav-icon">{s.icon}</span><span>{s.title}</span>
              {active === i && <div className="fl-guide-nav-bar" />}
            </button>
          ))}
        </nav>
        <div className="fl-guide-content">
          <div className="fl-guide-section-title"><span className="fl-guide-section-icon">{sections[active].icon}</span><h2>{sections[active].title}</h2></div>
          <div className="fl-guide-steps">
            {sections[active].steps.map((step, i) => (
              <div key={i} className="fl-guide-step">
                ...
                <div className="fl-guide-step-body"><h3>{step.title}</h3><p>{step.body}</p></div>
              </div>
            ))}
          </div>
          {active === 0 && (
            <div className="fl-guide-ref">
              ...
              <div className="fl-guide-ref-grid">
                {[{ label: 'FIRE Number', value: 'Annual expenses × 25', color: 'var(--gold)' }, { label: 'Return assumption', value: '7% per year', color: 'var(--purple-light)' }, { label: 'Safe withdrawal rate', value: '4% per year', color: 'var(--green)' }, { label: 'Target savings rate', value: '20%+ (FIRE: 40%+)', color: 'var(--blue)' }].map((r, i) => (
                  <div key={i} className="fl-guide-ref-item"><span className="fl-guide-ref-label">{r.label}</span><span className="fl-guide-ref-value" style={{ color: r.color }}>{r.value}</span></div>
                ))}
              </div>
            </div>
          )}
...
export default function AppDashboard() {
  ...
  const load = useCallback(async () => {
    ...
    if (isLifetime) {
      ...
      if (!sessionOnboarded) setShowOnboard(true);
      setLoading(false);
      return;
    }
  ...
  const deleteAllData = async () => {
    if (isLifetime) { setTxs([]); setDeleteConfirm(false); showToast('Session data cleared'); return; }
    await Promise.all([supabase.from('transactions').delete().eq('user_id', userId), supabase.from('user_settings').delete().eq('user_id', userId)]);
    setTxs([]); setFire({ annualExpenses: 40000, annualSavings: 20000, currentSavings: 50000 }); setCats(DEFAULT_CATS);
    setDeleteConfirm(false); showToast('All data deleted', 'error');
    sessionStorage.removeItem(`fl_onboarded_${userId}`); setOnboardStep(0); setShowOnboard(true);
  };
  ...
  const handleImportFile = e => {
    ...
    if (file.name.toLowerCase().endsWith('.pdf')) {
      reader.onload = ev => {
        ...
        const txLines = raw.split('\n').map(l => l.trim()).filter(Boolean).filter(l => /\d{1,2}[/-]\d{1,2}[/-]\d{2,4}/.test(l) && /\d+\.\d{2}/.test(l));
        if (txLines.length === 0) { showToast('PDF could not be read — please export as CSV from your bank instead', 'error'); return; }
        const pseudoCSV = 'Date,Description,Amount\n' + txLines.map(l => {
          const dateM = l.match(/\d{1,2}[/-]\d{1,2}[/-]\d{2,4}/); const amtM = l.match(/(\d[\d,]*\.\d{2})/g);
          const date = dateM ? dateM[0] : ''; const amount = amtM ? amtM[amtM.length - 1].replace(/,/g, '') : '0';
          return `${date},"${l.replace(date, '').replace(amount, '').replace(/[",]/g, ' ').trim().slice(0, 80)}",${amount}`;
        }).join('\n');
        processText(pseudoCSV);
      };
      reader.readAsText(file);
    } else { reader.onload = ev => processText(ev.target.result); reader.readAsText(file); }
  };
  ...
  const exportExcel = () => {
    ...
    const rows = [['FIRE LEDGER — FINANCIAL REPORT', '', '', '', '', ''], [`Currency: ${currency} · Generated: ${new Date().toLocaleDateString()}`, '', '', '', '', ''], ['', '', '', '', '', ''], ['FIRE NUMBER', fc.fireNum, '', 'CURRENT SAVINGS', fire.currentSavings, ''], ['YEARS TO FIRE', fc.years === Infinity ? 'N/A' : fc.years, '', 'PROGRESS', `${fc.progress.toFixed(1)}%`, ''], ['ANNUAL EXPENSES', fire.annualExpenses, '', 'ANNUAL SAVINGS', fire.annualSavings, ''], ['', '', '', '', '', ''], ['TOTAL INCOME', inc, '', 'SAVINGS RATE', inc > 0 ? `${((txs.filter(t => t.type === 'saving').reduce((s, t) => s + t.amount, 0) / inc) * 100).toFixed(1)}%` : 0, ''], ['', '', '', '', '', ''], ['Date', 'Description', 'Type', 'Category', 'Amount', 'Recurring'], ...txs.map(t => [t.date, t.description, t.type, t.category || '', t.amount, t.recurring ? 'Yes' : 'No'])];
  ...
  const activeModeCalc = calcFIRE(fireMode === 'lean' ? fire.annualExpenses * 0.75 : fireMode === 'fat' ? fire.annualExpenses * 1.5 : fireMode === 'barista' ? baristaNetExp : fire.annualExpenses, fire.annualSavings, fire.currentSavings);
    ...
    <div className="fl-shell">
      ...
      {milestone && (
        <div className="fl-milestone" onClick={() => setMilestone(null)}>
          <Icon.CheckCircle /><span>{milestone}</span><button><Icon.X /></button>
        </div>
      )}
        ...
        <div className="fl-overlay">
          <div className="fl-onboard">
            ...
            {onboardStep === 2 && (
              <div className="ob-step">
                ...
                <div style={{ display: 'flex', gap: 12, marginBottom: 16 }}>
                  <div className="ob-input-wrap" style={{ flex: 1 }}><span className="ob-sym">Now</span><input className="fl-input-lg ob-input" type="number" placeholder="30" value={currentAge || ''} onChange={e => setCurrentAge(parseInt(e.target.value) || 0)} /></div>
                  <div className="ob-input-wrap" style={{ flex: 1 }}><span className="ob-sym">Goal</span><input className="fl-input-lg ob-input" type="number" placeholder="45" value={retireAge || ''} onChange={e => setRetireAge(parseInt(e.target.value) || 0)} /></div>
                </div>
                <div className="ob-hint">{retireAge > currentAge ? `You want to stop working in ${retireAge - currentAge} years, at age ${retireAge}` : 'Enter your current age and target retirement age'}</div>
            ...
            {onboardStep === 4 && (
              <div className="ob-step">
                <h2>Your savings</h2>
                <p>Annual savings and total invested so far.</p>
                <div className="ob-input-wrap"><span className="ob-sym">{sym}</span><input className="fl-input-lg ob-input" type="number" placeholder="Annual savings" value={fire.annualSavings || ''} onChange={e => setFire(p => ({ ...p, annualSavings: parseFloat(e.target.value) || 0 }))} /></div>
                <div className="ob-input-wrap" style={{ marginTop: 10 }}><span className="ob-sym">{sym}</span><input className="fl-input-lg ob-input" type="number" placeholder="Current savings" value={fire.currentSavings || ''} onChange={e => setFire(p => ({ ...p, currentSavings: parseFloat(e.target.value) || 0 }))} /></div>
                ...
                <button className="fl-btn-primary" style={{ marginTop: 12 }} onClick={async () => {
                  ...
                  setShowOnboard(false); showToast('Profile saved'); setTimeout(() => setTourStep(0), 300);
            ...
            <div className="fl-fire-hero">
              <div className="fl-fire-hero-left">
                ...
                <div className="fl-fire-years">
                  ...
                  <span className="fl-fire-years-unit">{fireMode === 'coast' && coastReached ? ' coast reached' : activeModeYears !== Infinity ? ' years away' : ''}</span>
                </div>
                ...
                {fireMode === 'barista' && (
                  <div className="fl-fire-barista-note">Work part-time earning {fmt(baristaPartTimeIncome)}/yr · portfolio covers the rest</div>
                )}
            ...
            <div key="fire" className="fl-page">
              <div className="fl-page-top">
                ...
                <div className="fl-fire-mode-tabs">
                  {[{ id: 'standard', label: 'FIRE' }, { id: 'lean', label: 'Lean FIRE' }, { id: 'barista', label: 'Barista FIRE', icon: <Icon.Coffee /> }, { id: 'coast', label: 'Coast FIRE' }, { id: 'fat', label: 'Fat FIRE' }].map(m => (
              ...
              <div className="fl-fire-mode-desc">
                ...
                {fireMode === 'lean' && <p><strong>Lean FIRE</strong> — Retire on 75% of your current expenses. FIRE number = {fmt(leanNum)}.</p>}
                {fireMode === 'fat' && <p><strong>Fat FIRE</strong> — Retire on 150% of your current expenses. FIRE number = {fmt(fatNum)}.</p>}
                ...
                <div className="fl-fire-inputs">
                  ...
                  <div className="fl-fire-field">
                    ...
                    <div style={{ display: 'flex', gap: 8 }}>
                      <div className="fl-fire-input-wrap" style={{ flex: 1 }}><span className="fl-input-prefix">Now</span><input className="fl-fire-input" type="number" placeholder="30" value={currentAge || ''} onChange={e => setCurrentAge(parseInt(e.target.value) || 0)} /></div>
                      <div className="fl-fire-input-wrap" style={{ flex: 1 }}><span className="fl-input-prefix">Goal</span><input className="fl-fire-input" type="number" placeholder="45" value={retireAge || ''} onChange={e => setRetireAge(parseInt(e.target.value) || 0)} /></div>
                    </div>
                  ...
                  {[{ key: 'annualExpenses', label: 'Annual Expenses', hint: 'Expected yearly spend in retirement' }, { key: 'annualSavings', label: 'Annual Savings', hint: 'Amount you invest per year' }, { key: 'currentSavings', label: 'Current Savings', hint: 'Total already invested or saved' }].map(f => (
                    <div key={f.key} className="fl-fire-field">
                      <label>{f.label}</label>
                      <div className="fl-fire-input-wrap"><span className="fl-input-prefix">{sym}</span><input className="fl-fire-input" type="number" placeholder="0" value={fire[f.key] || ''} onChange={e => setFire(p => ({ ...p, [f.key]: parseFloat(e.target.value) || 0 }))} /></div>
                      <span className="fl-fire-hint">{f.hint}</span>
                    </div>
                  ))}
                  <div className="fl-fire-field">
                    <label>Inflation Rate</label>
                    <div className="fl-fire-input-wrap"><span className="fl-input-prefix">%</span><input className="fl-fire-input" type="number" placeholder="3" step="0.5" value={inflation || ''} onChange={e => setInflation(parseFloat(e.target.value) || 0)} /></div>
                    <span className="fl-fire-hint">Real return adjusted for inflation (default 3%)</span>
                  </div>
                  <div className="fl-fire-field">
                    <label>Safe Withdrawal Rate</label>
                    <div className="fl-fire-input-wrap"><span className="fl-input-prefix">%</span><input className="fl-fire-input" type="number" placeholder="4" step="0.1" value={swr || ''} onChange={e => setSwr(parseFloat(e.target.value) || 4)} /></div>
                  ...
                  {fireMode === 'coast' && (
                    <div className="fl-fire-field">
                      ...
                      <div className="fl-fire-input-wrap" style={{ marginTop: 6 }}><span className="fl-input-prefix">{sym}</span><input className="fl-fire-input" type="number" placeholder="0" value={contribs || ''} onChange={e => setContribs(parseFloat(e.target.value) || 0)} /></div>
                  ...
                  {fireMode === 'barista' && (
                    <div className="fl-fire-field">
                      ...
                      <div className="fl-fire-input-wrap"><span className="fl-input-prefix">{sym}</span><input className="fl-fire-input" type="number" readOnly value={Math.round(baristaPartTimeIncome)} style={{ opacity: 0.6 }} /></div>
                ...
                <div className="fl-fire-results">
                  <div className="fl-fire-big-ring">
                    <svg viewBox="0 0 200 200">
                      ...
                      <circle cx="100" cy="100" r="85" fill="none" stroke="url(#fire-calc-grad)" strokeWidth="16" strokeDasharray="534" strokeDashoffset={534 - (534 * displayProgress / 100)} strokeLinecap="round" transform="rotate(-90 100 100)" />
                      <defs><linearGradient id="fire-calc-grad" x1="0%" y1="0%" x2="100%" y2="100%"><stop offset="0%" stopColor="var(--purple-light)" /><stop offset="100%" stopColor="var(--purple-dark)" /></linearGradient></defs>
                    </svg>
                  ...
                  <div className="fl-fire-stat-grid">
                    {(() => {
                      ...
                      const stats = fireMode === 'coast' ? [
                        ...
                        { label: 'Age at Freedom', value: coastReached ? currentAge : displayAgeAtFIRE, hint: 'Age when you stop working', color: 'var(--purple-light)' },
          ...
          <div key="proj" className="fl-page">
            ...
            <div className="fl-mc-card">
              ...
              {mcResult && <>
                ...
                <p className="fl-mc-disclaimer"><strong>Disclaimer:</strong> Monte Carlo simulations are for illustrative purposes only and do not constitute financial advice. Past market performance does not guarantee future results. Consult a qualified financial adviser before making investment or retirement decisions.</p>
              </>}
          ...
          <div key="export" className="fl-page">
            ...
            <div className="fl-export-grid">
              ...
              <div className="fl-export-card">
                ...
                <p>Import from any CSV or PDF bank statement. Auto-detects column layouts, date formats, and debit/credit columns. No reformatting required.</p>
            ...
            <div className="fl-nw-layout">
              <div className="fl-nw-col">
                <div className="fl-nw-section fl-nw-assets">
                  <h3>Assets</h3>
                  {[{ key: 'houseValue', label: 'Property Value' }, { key: 'carValue', label: 'Vehicle Value' }, { key: 'cashSavings', label: 'Cash & Savings' }, { key: 'investments', label: 'Investments' }, { key: 'otherAssets', label: 'Other Assets' }].map(f => (
                    <div key={f.key} className="fl-nw-field"><label>{f.label}</label><div className="fl-fire-input-wrap"><span className="fl-input-prefix">{sym}</span><input className="fl-fire-input" type="number" placeholder="0" value={nw[f.key] || ''} onChange={e => setNw(p => ({ ...p, [f.key]: parseFloat(e.target.value) || 0 }))} /></div></div>
                  ))}
                  <div className="fl-nw-subtotal fl-nw-subtotal-green"><span>Total Assets</span><strong>{fmt(Object.entries(nw).filter(([k]) => ['houseValue', 'carValue', 'cashSavings', 'investments', 'otherAssets'].includes(k)).reduce((s, [, v]) => s + v, 0))}</strong></div>
                </div>
              </div>
              <div className="fl-nw-col">
                <div className="fl-nw-section fl-nw-liabilities">
                  <h3>Liabilities</h3>
                  {[{ key: 'mortgage', label: 'Mortgage' }, { key: 'creditCard', label: 'Credit Card Debt' }, { key: 'studentLoan', label: 'Student Loan' }, { key: 'personalLoan', label: 'Personal Loan' }, { key: 'otherLiabilities', label: 'Other Liabilities' }].map(f => (
                    <div key={f.key} className="fl-nw-field"><label>{f.label}</label><div className="fl-fire-input-wrap"><span className="fl-input-prefix">{sym}</span><input className="fl-fire-input" type="number" placeholder="0" value={nw[f.key] || ''} onChange={e => setNw(p => ({ ...p, [f.key]: parseFloat(e.target.value) || 0 }))} /></div></div>
                  ))}
                  <div className="fl-nw-subtotal fl-nw-subtotal-red"><span>Total Liabilities</span><strong>{fmt(Object.entries(nw).filter(([k]) => ['mortgage', 'creditCard', 'studentLoan', 'personalLoan', 'otherLiabilities'].includes(k)).reduce((s, [, v]) => s + v, 0))}</strong></div>
                </div>
              </div>
            </div>
            {(() => {
              ...
              return (
                <div className="fl-nw-result">
                  <div className="fl-nw-result-main"><span className="fl-nw-result-label">Net Worth</span><span className="fl-nw-result-value" style={{ color: netWorth >= 0 ? 'var(--green)' : 'var(--red)' }}>{fmt(netWorth)}</span></div>
                  ...
                  <div className="fl-nw-bar-wrap">
                    <div className="fl-nw-bar-track"><div className="fl-nw-bar-assets" style={{ width: `${assets > 0 ? Math.min((assets / (assets + liabs)) * 100, 100) : 100}%` }} /></div>
          ...
          <div key="compound" className="fl-page">
            ...
            <div className="fl-cg-layout">
              <div className="fl-cg-inputs">
                <h3>Your Numbers</h3>
                <div className="fl-fire-field"><label>Initial Investment</label><div className="fl-fire-input-wrap"><span className="fl-input-prefix">{sym}</span><input className="fl-fire-input" type="number" placeholder="0" value={cg.initial || ''} onChange={e => setCg(p => ({ ...p, initial: parseFloat(e.target.value) || 0 }))} /></div></div>
                <div className="fl-fire-field"><label>Investment Timeline</label><div className="fl-fire-input-wrap"><span className="fl-input-prefix">yr</span><input className="fl-fire-input" type="number" placeholder="10" value={cg.years || ''} onChange={e => setCg(p => ({ ...p, years: parseInt(e.target.value) || 0 }))} /></div></div>
                <div className="fl-fire-field"><label>Annual Growth Rate</label><div className="fl-fire-input-wrap"><span className="fl-input-prefix">%</span><input className="fl-fire-input" type="number" placeholder="7" step="0.5" value={cg.growthRate || ''} onChange={e => setCg(p => ({ ...p, growthRate: parseFloat(e.target.value) || 0 }))} /></div></div>
                <div className="fl-fire-field"><label>Annual Inflation Rate</label><div className="fl-fire-input-wrap"><span className="fl-input-prefix">%</span><input className="fl-fire-input" type="number" placeholder="3" step="0.5" value={cg.inflationRate || ''} onChange={e => setCg(p => ({ ...p, inflationRate: parseFloat(e.target.value) || 0 }))} /></div></div>
                <div className="fl-fire-field">
                  <label>Contributions</label>
                  <div className="fl-contrib-toggle">
                  ...
                  <div className="fl-fire-input-wrap" style={{ marginTop: 6 }}><span className="fl-input-prefix">{sym}</span><input className="fl-fire-input" type="number" placeholder="0" value={cg.contribution || ''} onChange={e => setCg(p => ({ ...p, contribution: parseFloat(e.target.value) || 0 }))} /></div>
                </div>
                <div className="fl-fire-field">
                ...
                <button className="fl-btn-primary" style={{ width: '100%' }} onClick={() => {
                  ...
                  setCgResult({ nominal, real, totalContribs, growth: nominal - totalContribs, rule72: cg.growthRate > 0 ? (72 / cg.growthRate).toFixed(1) : '—' });
                ...
                {cgResult ? (
                  <>
                    ...
                    <div className="fl-fire-stat-grid" style={{ marginTop: 16 }}>
                      {[{ label: 'Real Value', value: fmt(cgResult.real), hint: 'Inflation-adjusted', color: 'var(--green)' }, { label: 'Total Contributed', value: fmt(cgResult.totalContribs), hint: 'Your actual money in', color: 'var(--t1)' }, { label: 'Growth', value: fmt(cgResult.growth), hint: 'Compound interest earned', color: 'var(--gold)' }, { label: 'Rule of 72', value: `${cgResult.rule72} yrs`, hint: 'Years to double at this rate', color: 'var(--purple-light)' }].map((s, i) => (
                        <div key={i} className="fl-fire-stat"><span className="fl-fire-stat-label">{s.label}</span><span className="fl-fire-stat-value" style={{ color: s.color, fontSize: 18 }}>{s.value}</span><span className="fl-fire-stat-hint">{s.hint}</span></div>
                      ))}
                    </div>
                    <div className="fl-cg-disclaimer">For illustrative purposes only. Does not constitute financial advice.</div>
                  </>
                ) : (
                  <div className="fl-cg-empty"><div className="fl-cg-empty-icon"><Icon.Lightning /></div><p>Enter your numbers and click Calculate Growth to see how your investment compounds over time.</p></div>
                )}
              </div>
            </div>
          </div>
            ...
            <div className="fl-settings-grid">
              <div className="fl-settings-card">
                ...
                <div className="fl-danger-zone" style={{ marginTop: 24 }}>
                  ...
                  <button className="fl-btn-danger fl-btn-icon" style={{ width: '100%' }} onClick={() => setDeleteConfirm(true)}><Icon.Trash />Delete all my data</button>
                </div>
              </div>
              <div className="fl-settings-card">
                <h3>Currency</h3>
                ...
                <div style={{ display: 'flex', gap: 12, marginBottom: 16 }}>
                  <div className="fl-fire-field" style={{ flex: 1, marginBottom: 0 }}>
                    ...
                    <div className="fl-fire-input-wrap"><span className="fl-input-prefix">yr</span><input className="fl-fire-input" type="number" placeholder="30" value={currentAge || ''} onChange={e => setCurrentAge(parseInt(e.target.value) || 0)} /></div>
                  </div>
                  <div className="fl-fire-field" style={{ flex: 1, marginBottom: 0 }}>
                    <label>Retirement Age Goal</label>
                    <div className="fl-fire-input-wrap"><span className="fl-input-prefix">yr</span><input className="fl-fire-input" type="number" placeholder="45" value={retireAge || ''} onChange={e => setRetireAge(parseInt(e.target.value) || 0)} /></div>
                  </div>
                </div>
                ...
                {isLifetime && (
                  <div className="fl-upgrade-card">
                    <div className="fl-upgrade-card-top"><span className="fl-upgrade-card-title">Upgrade to cloud sync</span><span className="fl-upgrade-card-badge">Save your data</span></div>
                    <p className="fl-upgrade-card-body">Your data clears every session on the Lifetime plan. Upgrade to Monthly or Annual to save everything permanently.</p>
                    <div className="fl-upgrade-card-options">
                      <a href="/pricing" className="fl-upgrade-option"><span className="fl-upgrade-option-price">$4.99<span>/mo</span></span><span className="fl-upgrade-option-label">Monthly</span></a>
                      <a href="/pricing" className="fl-upgrade-option fl-upgrade-option-featured"><span className="fl-upgrade-option-price">$59.99<span>/yr</span></span><span className="fl-upgrade-option-label">Annual · Best value</span></a>
                    </div>
                  </div>
                )}
